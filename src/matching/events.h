...
namespace exchange::matching {
    ...
    struct Trade {
        SequenceNumber sequence_number;
        Timestamp timestamp;
        // one order can have multi-match ids
        MatchId match_id;
        Symbol symbol;
    ...
    struct Event {
        // which member of union is currently valid
        EventType type{ EventType::ORDER_REJECTED };

        union Payload {
            // just to do a OR (at a time only one is active)
        ...
        static Event make(const OrderAccepted& value) noexcept {
            ...
            // to get the values of payload of that type
